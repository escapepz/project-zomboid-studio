...
## TODO
- What if we keep `.docs` and `.libraries` in a central `dist/` directory?
Every time `pzstudio new` is run, the CLI could create a *Directory Junction* via `mklink /J`. Just like `pnpm`, this ensures a single source of truth and faster workflows by avoiding repetitive git operations.
