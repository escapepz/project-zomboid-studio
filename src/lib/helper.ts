...
export function workingDir() {
    return basename(__dirname) === 'dist' ? __dirname : join(dirname(__dirname), 'lib');
}
...
export function templateDir(
    ...
    const root = basename(__dirname) === 'dist' ? join(__dirname, '..') : join(dirname(__dirname), '..');
...
export function updateExperimentalScripts(
    ...
    try {
        const scriptPath = basename(__dirname) === 'dist'
            ? resolve(__dirname, 'scripts/experimental-package-scripts.js')
            : resolve(dirname(__dirname), '../scripts/experimental-package-scripts.js');
